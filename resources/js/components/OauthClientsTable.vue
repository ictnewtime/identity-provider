        ...
        <div class="table-responsive min-height">
            <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    <oauth-row
                        v-for="client in pagination.data"
                        :oauth_client_id="client.oauth_client_id"
                        :oauth_name="client.oauth_name"
                        :oauth_roles="client.roles"
                        :loadClients="(page) => loadClients((page = pagination.meta.current_page))"
                        :key="client.oauth_client_id"
                        :roles="roles"
                    ></oauth-row>
                    <tr v-if="!loading && pagination.data.length === 0">
                        <td colspan="7" class="text-center">Nessun client trovato</td>
                    </tr>
                </tbody>
            </table>
        </div>
        <paginator
            v-if="pagination.meta.total > pagination.meta.per_page"
            :pagination="pagination.meta"
            :onChangePage="(page) => loadClients(page)"
        ></paginator>
    </div>
</template>
