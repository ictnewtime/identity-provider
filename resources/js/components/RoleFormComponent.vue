    ...
    <div id="create-role-container" class="pb-4">
        ...
        <form class="form-inline mt-4">
            ...
            <input
                v-model="roleName"
                type="text"
                :class="'form-control mb-2 mr-sm-2 col-12 col-lg-4 ' + (!roleState ? 'is-invalid' : '')"
                id="input-role-name"
                placeholder="Inserisci un nuovo ruolo"
            />
        ...
        <div v-if="loading" class="pr-3"><i class="fas fa-spinner fa-pulse"></i>Caricamento in corso...</div>
        <role-chip
            v-for="role in roles"
            :id="role.id"
            :name="role.name"
...
export default {
    ...
    methods: {
        fetchRoles() {
            this.loading = true;
            let vm = this;
            const url = window.location.origin + "/admin/v1/roles";
            axios
                .get(url)
                .then((data) => {
                    vm.roles = data.data;
        ...
        deleteRole(id) {
            ...
            return new Promise((resolve, reject) => {
                const url = window.location.origin + `/admin/v1/roles/${id}`;
                axios
                    .delete(url)
                    .then((response) => {
