<template>
    <div class="d-inline-flex align-items-center chip mb-3 mr-2">
        ...
        <div
            v-if="!requestDelete && !loading"
            class="d-flex justify-content-center align-items-center pl-1 pr-3 icon-close"
        >
            <div>
...
export default {
    ...
    methods: {
        ...
        deleteChipRole() {
            let vm = this;
            this.loading = true;
            const url = window.location.origin + `/admin/v1/provider-user-roles/has-relation?role_id=${vm.id}`;
            console.log("url", url);
