...
| TCC15 | da approvare | **Due database locali, e non uno solo.** Oggi ne esiste uno — `idp_staging` — che fa da database di sviluppo **e** finisce per essere quello che i test distruggono (`VDF11`). Vanno separati: **`idp_local`** per lo sviluppo e **`idp_test`** per la suite, sulla stessa istanza MariaDB. I test non toccano più il lavoro di chi sviluppa, e riseminare il primo non ha effetto sul secondo. Il nome `idp_staging` per un database locale è per giunta fuorviante | `docker-compose.yml` (variabile del servizio `mariadb`), `.env` e `.env.dev` (`DB_DATABASE=idp_local`), `phpunit.xml` (connessione di test su `idp_test`), `docs/setup.db.md` | medio — cambia l'ambiente di ogni sviluppatore, e va fatto in una volta sola | man | con la suite in corso, `idp_local` resta popolato; `idp_test` è l'unico che viene ricreato |
| TCC14 | da approvare | **`VDF11` — la difesa, che vale anche se `TCC15` non c'è.** La config cache rende `env()` inerte, quindi `phpunit.xml` **non protegge niente**: serve un controllo che non dipenda da `env()`. Un `TestCase::setUp()` che **aborta** se il database in uso non è quello di test — sqlite in memoria oppure `idp_test` — con un messaggio che dice cosa fare. Vale per ogni test futuro, compresi quelli che nessuno ha ancora scritto | `tests/TestCase.php`, `docs/TEST.md` | **alto** — è il difetto che ha già fatto danno | auto | con la config cache presente e `DB_DATABASE=idp_local`, la suite **si ferma** con un messaggio invece di migrare il database di sviluppo |
