...
| TFC06 | da approvare | **La verifica che chiude i rilievi**: al primo scan le tre funzioni sono sotto 15. È l'unica prova che il rilievo è chiuso — e **non dice niente sul comportamento**, che è ciò per cui esistono `TFC03` e `TFC05` | nessuno (verifica) | basso | man | i tre rilievi `high` non compaiono più nel report |
...
- **Le specifiche Cypress** (`D4`, `F5`): `ProviderForm` non ha più una specifica E2E — quella che
  ...
  Riscriverle è lavoro di [e2e-test-container](../20260812-e2e-test-container/), non di qui.
