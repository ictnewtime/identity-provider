...
| TSR15 | da approvare | **Il clone nuovo: è qui che il ciclo riparte, e nessun punto precedente lo copre.** `vendor`, `node_modules` e `public/build` sono in `.gitignore` (`:25`, `:19`, `:20`), quindi un clone non li ha; il montaggio `./:/var/www` copre quelli dell'immagine, quindi l'applicazione non li trova; e chi li installa lo fa **dentro** un container che è root — file di root, da capo. Intervento: `entrypoint.sh` installa le dipendenze **se mancano**, lasciando i privilegi — `su -s /bin/sh www-data` o l'`HOST_UID` di `TSR13` — invece di lasciare che lo faccia una persona come root. Effetto secondario che è il vero guadagno: un clone nuovo parte con `docker compose up` e basta | `entrypoint.sh`, `docker-compose.yml` | **medio — tocca l'avvio dell'ambiente** | man | in una cartella vergine (`git clone` + `docker compose up`), l'applicazione risponde su `:8001` e `find vendor -user root` non trova niente |
