...
> [e2e-test-container](../../todo/20260812-e2e-test-container/action-plan.md). Sono `fatto` per decisione del
...
| TSA14 | **fatto** (2026-08-12) — eseguito dal developer; verificato: `git ls-files cypress.env.json` non stampa niente e il file esiste ancora sul disco | **Sganciare `cypress.env.json` dal tracciamento git** — è già in `.gitignore` (ultima riga) ma resta indicizzato, quindi continua a essere distribuito a ogni clone. Il comando è `git rm --cached cypress.env.json` e **lo esegue il developer** (R2: git in sola lettura per l'agente); il file sul disco resta dov'è | `cypress.env.json` (solo l'indice git) | basso, dopo `D1` | auto | `git ls-files cypress.env.json` non stampa niente, e il file esiste ancora sul disco |
...
| TSA10a | **fatto** (2026-08-12) — la verifica funzionale e' delegata a `TEC04` di [e2e-test-container](../../todo/20260812-e2e-test-container/action-plan.md) | **Una riga** in `docs/SETUP.md`, subito dopo il blocco di `php artisan key:generate`, che invoca lo script e lo dichiara obbligatorio. Solo l'invocazione: la procedura sta nello script, non nel documento — due copie divergono, e il documento è quella che nessuno riesegue | `docs/SETUP.md` | basso | auto | il documento cita `scripts/prepare-e2e-credentials.sh` e **non** contiene comandi di generazione né valori |
| TSA11 | **fatto** (2026-08-12) — la verifica funzionale e' delegata a `TEC04` di [e2e-test-container](../../todo/20260812-e2e-test-container/action-plan.md) | `E2EUserSeeder` che crea `e2e.admin` ed `e2e.user` leggendo le password **dalle variabili d'ambiente**, senza valore di ripiego scritto nel codice. Chiude la seconda metà di `TSA10`, che senza di lui resta dichiarata e non funzionante | `database/seeders/E2EUserSeeder.php` (nuovo) | medio — crea utenti con privilegi di amministratore | auto | eseguito lo script, `npm run cy:run -- --spec cypress/e2e/auth/login.cy.js` passa; senza le variabili esportate il seeder **fallisce** invece di inventare una password |
| TSA12 | **fatto** (2026-08-12) — la verifica funzionale e' delegata a `TEC04` di [e2e-test-container](../../todo/20260812-e2e-test-container/action-plan.md) | Togliere la password scritta in chiaro nel seeder principale (`F14`, difetto `VDF08`): `admin.admin` prende la sua da una variabile d'ambiente, come gli utenti E2E | `database/seeders/DatabaseSeeder.php:39` | medio — cambia la preparazione di ogni ambiente nuovo | auto | `grep -n 'Hash::make("' database/seeders/` non trova più letterali |
| TSA03 | **fatto** (2026-08-12) — la verifica funzionale e' delegata a `TEC04` di [e2e-test-container](../../todo/20260812-e2e-test-container/action-plan.md) | Spostare la password dei dati di test da letterale a `Cypress.env(...)`, con la chiave dichiarata in `cypress.env.example.json` e il **valore generato** da `TSA10`. **Dopo `TSA10`**: prima che lo script esista, la chiave non ha chi la riempia | `cypress/e2e/user/crud-user.cy.js:11` | basso | auto | `grep -nE 'password:\s*"' cypress/e2e/` non trova più letterali (R6: il valore non si riporta, si nomina il campo); `npm run cy:run -- --spec cypress/e2e/user/crud-user.cy.js` passa |
| TSA15 | **fatto** (2026-08-12) | `docs/TEST.md`: come si eseguono le due famiglie di test — PHPUnit **dentro** il container, Cypress **fuori** — e la tabella di cosa manca all'immagine perche' Cypress ci giri dentro. Nasce dalla verifica manuale di `TSA10`: senza un posto dove sta scritto come si lancia Cypress, i punti sotto non si possono chiudere | `docs/TEST.md` (nuovo) | basso | auto | il documento esiste e le sue affermazioni sull'immagine sono verificabili con `dpkg -s` |
...
| ID | Stato | Punto | File toccati | Rischio | V | Come si verifica |
|---|---|---|---|---|---|---|
| TSA04 | **fatto** (2026-08-12) | `Number.parseInt(…, 10)` con la radice e il ripiego **fuori** dalla chiamata, ma controllando `Number.isNaN` invece della verita': `0` e' un valore legittimo (domenica, in `en`), e un `\|\| 1` fuori lo avrebbe trasformato in `1` — cioe' avrebbe **rotto la locale inglese** correggendo il difetto latente | `resources/js/ui/LocalizedDatePicker.vue:11-17` | basso | auto | **verifica cambiata**: il test unitario del piano non e' scrivibile (nel progetto non c'e' nessun runner JS e `D5` esclude nuove dipendenze di sviluppo — `BDB24`). Verificato invece con un banco di prova su quattro casi — `it` → 1, `en` → 0, chiave mancante → 1, valore vuoto → 1 — e la sintassi del blocco `<script setup>`. `npm run build` **non e' eseguibile**: fallisce per un motivo estraneo, `BDB25` |
