...
| TCC04 | **fatto** (2026-08-13) | **`F7`** — tetto a `per_page`, validato, con massimo esplicito: oggi il valore arriva dal client senza limite su una tabella che cresce di continuo (punto 5 della checklist perf/leak) | `AuditController.php:70` | medio | auto | test: `?per_page=1000000` restituisce al più il massimo consentito |
| TCC05 | **fatto** (2026-08-13) | **`F9`, `D4`** — **prima un test unitario** che stabilisca cosa fa `->latest()` dopo un `orderBy` esplicito e con la join attiva: ambiguità su `created_at` o risoluzione sulla lista di output? **Poi** togliere la riga. Il test resta: è la regressione che documenta il perché | `tests/Unit/AuditOrderingTest.php` (nuovo), poi `AuditController.php:73` | basso se è pulizia, **alto** se il test scopre un errore su una colonna ordinabile | auto | il test dichiara il comportamento osservato; dopo la rimozione resta verde e l'ordine predefinito è ancora il più recente per primo |
| TCC08 | **fatto** (2026-08-13) | **`F6c`, `F6d`** — fissare la regola **finché nessuno ha ancora scritto quel codice**: un client Passport **si revoca, non si cancella**. `Passport\Client` non ha soft delete e una cancellazione lascerebbe gli audit orfani del loro attore. La colonna `revoked` esiste già; oggi l'applicazione non cancella client, quindi il costo è zero e il momento è adesso | `docs/doc-code-guide-line.md`; eventuale guardia dove nascerà la cancellazione | basso oggi, **alto quando qualcuno scriverà il metodo** senza saperlo | man | la regola è scritta dove chi implementerà la cancellazione la incontra |
| TCC09 | **fatto** (2026-08-13) | **`D3`** — l'assenza di uno scope per tenant è una **scelta**: in questo sistema i tenant non si gestiscono e un amministratore vede tutti gli audit. **La sede è cambiata su indicazione del developer**: non un commento nel controller, ma il nuovo [docs/project-analysis.md](/docs/project-analysis.md), che raccoglierà un po' alla volta la documentazione del progetto. Nel controller resta **una riga di rimando**, perché chi legge la query deve poter arrivare al perché | `docs/project-analysis.md` (nuovo), `AuditController.php:26-28` | nullo | man | il documento dice che è una scelta e cosa comporterebbe cambiarla; il controller ci rimanda |
...
| ID | Stato | Punto | File toccati | Rischio | V | Come si verifica |
|---|---|---|---|---|---|---|
| TCC06 | **fatto** (2026-08-13) | `AuditController::all()` scomposto in **tre classi**: `AuditSearchFilter`, `AuditSortOrder` e `AuditListQuery` che le compone (`app/Queries/Audit/`). Il controller passa da 61 a **poche righe** e non parla più direttamente con l'ORM. Unit test per classe che asseriscono sull'**SQL prodotto**, senza database: e' l'unico modo di distinguere «cerca su quattro campi» da «cerca su tre e sembra funzionare» quando i dati di prova non coprono il quarto | `app/Queries/Audit/` (3 classi nuove), `AuditController.php` | basso | man | i 16 test di `TCC02` verdi **senza una modifica**; 9 unit test nuovi sulle due classi; il numero riletto in SonarQube |
| TCC07 | **fatto** (2026-08-13) | `Authenticated::handle()` scomposto per **fase di validazione** in tre classi (`app/Auth/Idp/`): `IdpTokenExtractor` — l'unica che non tocca niente, quindi unit test puro — `IdpProviderResolver` e `IdpSessionValidator`. Il `try` non avvolge più quasi tutto: circonda la sola decodifica, che è l'unica cosa che può lanciare. Decodifica e risoluzione dell'utente sono metodi privati con la loro uscita | `app/Auth/Idp/` (3 classi nuove), `Authenticated.php` | **alto** — percorso di autenticazione di tutta l'area protetta | man | gli 8 test di `TCC01` verdi **senza una modifica**; 6 unit test + 5 feature test sulle fasi; il numero riletto in SonarQube |
| TCC10 | **fatto** (2026-08-13) | **Cinque test di composizione** (`IdpCompositionTest`): il flusso intero; token valido con **sessione revocata**; sessione viva ma **utente sparito** — due fasi che si contraddicono, e deve vincere l'assenza dell'utente; token firmato da **un altro provider**; cookie e bearer **insieme**, dove vince il cookie. Sono gli attraversamenti, che i test per classe non coprono: ogni fase può essere corretta mentre il passaggio fra due è rotto | `tests/Feature/Auth/IdpCompositionTest.php` | medio | auto | 5 verdi. Uno ha trovato subito un attraversamento non ovvio: il client di test cifra i cookie anche quando `EncryptCookies` li esclude a runtime |
