...
| [./todo/20260828-master-token-sessions/](./todo/20260828-master-token-sessions/) | La riga in `sessions` scade insieme all'**app token** — trenta minuti — e chi la interroga disconnette. La proposta: legarla al **master token**, otto ore, e una rotta `v2` che accetta i token negli header. Le **undici implicazioni negative** del § 6 sono state lette una per una, e la lettura ha **cambiato la strada**: la (b) rompeva `validateSession()`, quindi la riga resta per utente+provider e porta il master token in `refresh_token` — la colonna che nessuno usava | `TMT` | alta |
