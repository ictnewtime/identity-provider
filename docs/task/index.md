...
|---|---|---|---|
| [./done/20260901-duplicated-test-literals/](./done/20260901-duplicated-test-literals/) | Cinque rilievi di SonarQube, tutti nello stesso posto: due file di test ripetono l'indirizzo IP di prova **23** volte e i percorsi delle due rotte di scambio **26**. Cambiare una rotta oggi vuol dire cambiarla in sedici righe. | `TDL` | bassa |
| [./todo/20260901-extension-auth-me/](./todo/20260901-extension-auth-me/) | Ogni applicazione si copia a mano la rotta `/me`, e i due esempi nella documentazione dell'estensione **non rispondono la stessa cosa**. I due pacchetti client — PHP e Node — devono portarsela dentro, come Passport porta le sue rotte, così chi installa ce l'ha senza scrivere niente. | `TAM` | media |
| [./todo/20260828-token-reuse-detection/](./todo/20260828-token-reuse-detection/) | «Se hanno rubato il master-token possono rinnovarlo all'infinito» — osservazione del developer, ed è esatta: senza rilevamento la rotazione **allunga** la vita di un token rubato. Il pezzo che serve esiste da oggi (`TMT02` mette il master token nella riga), e nessuno lo confronta. Il nodo è il falso positivo: con una riga per utente, il secondo dispositivo somiglia a un furto | `TRR` | media |
| [./done/20260828-master-token-sessions/](./done/20260828-master-token-sessions/) | La riga in `sessions` scade insieme all'**app token** — trenta minuti — e chi la interroga disconnette. La proposta: legarla al **master token**, otto ore, e una rotta `v2` che accetta i token negli header. Le **undici implicazioni negative** del § 6 sono state lette una per una, e la lettura ha **cambiato la strada**: la (b) rompeva `validateSession()`, quindi la riga resta per utente+provider e porta il master token in `refresh_token` — la colonna che nessuno usava | `TMT` | alta |
...
| [./done/20260820-security-reliability/](./done/20260820-security-reliability/) | Sei rilievi di sicurezza e affidabilità, più otto punti nati verificandoli. Il titolo della scheda non arrivava per **due `<title>`** nell'`head`; nove etichette erano collegate a un `div` invece che a un campo (`VDF25`); e i file di root nell'albero avevano **tre** sorgenti, non una — un comando a mano, vite avviato come root e il primo `install` su un clone nuovo. Chiuso con l'albero pulito e le tre sorgenti spente | `TSR` | 2026-08-21 |
| [./done/20260821-exceptions-and-waits/](./done/20260821-exceptions-and-waits/) | Sei rilievi `medium`: sette `RuntimeException` generiche diventate due eccezioni con un nome, un ternario annidato sciolto, e due `cy.wait(500)` che **coincidevano col debounce** sostituiti da un'attesa sulla richiesta. **Con una verifica sospesa**: i due spec Cypress sono scritti e mai eseguiti, si provano quando esiste il container E2E | `TEW` | 2026-08-21 |
| [./done/20260821-regex-and-counters/](./done/20260821-regex-and-counters/) | Sette rilievi di SonarQube. Il rilievo sulla complessità di due regex era il sintomo: quelle liste scritte a mano non riconoscevano **51 finestre su 94** — tutto l'alfabeto all'indietro, le righe di tastiera nei due versi, `0123` e `3210`. Ora le sequenze si **generano** dai codici dei caratteri, e la riflessione su una proprietà privata è sparita insieme al rilievo che la segnalava | `TRC` | 2026-08-21 |
| [./done/20260821-deprecated-string-api/](./done/20260821-deprecated-string-api/) | Tre rilievi su due righe — `String.fromCharCode` e `substr` — **introdotte dal lotto chiuso poche ore prima**. Chiuso con quattro verifiche invece di una: 20006 password a confronto, le 94 finestre ancora riconosciute, e i bordi dove un cambio di firma si vedrebbe | `TDS` | 2026-08-21 |
| [./done/20260813-token-refresh/](./done/20260813-token-refresh/) | Il rinnovo dell'app token. **Chiuso come rettificato**, non come fatto: l'analisi di `TMT` è più avanzata e le domande di qui là hanno una risposta. I sette punti che `TMT` non copriva sono stati portati là (`TMT08`…`TMT14`); l'analisi resta la descrizione più dettagliata del flusso dei due token | `TTR` | 2026-08-28 |
